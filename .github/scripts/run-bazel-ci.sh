...
windows_cross_compile_local_fallback=0
...
if [[ "${RUNNER_OS:-}" == "Windows" && $windows_cross_compile -eq 1 && -z "${BUILDBUDDY_API_KEY:-}" ]]; then
  # Windows gnullvm cross-compilation depends on authenticated RBE. Hosted
  # community runners use a fully MSVC local build when credentials are absent.
  ci_config=ci-windows
  windows_msvc_host_platform=1
  windows_cross_compile_local_fallback=1
fi
...
if [[ "${RUNNER_OS:-}" == "Windows" && $windows_cross_compile_local_fallback -eq 1 ]]; then
  post_config_bazel_args+=("--platforms=//:windows_x86_64_msvc")
fi
