...
Codez tracks upstream Codex releases and publishes unsigned packages for macOS and Linux x86_64/ARM64.
...
- `CODEX_RELEASE` pins a release such as `codez-v0.153.4-r1`.
...
tar -xzf codez-offline-v0.153.4-r1.tar.gz
./codez-offline-v0.153.4-r1/install-codez-offline.sh \
  --bundle ./codez-offline-v0.153.4-r1
```

The offline installer selects the matching macOS or Linux x86_64/ARM64 package,
verifies its bundled SHA-256 entry, and installs it
...
```shell
CODEX_REPO_ROOT="$PWD" CODEZ_VERSION=0.153.4-r1 python3 scripts/build_codex_package.py \
  --variant codez \
...
Tags use `codez-v<upstream-version>-r<N>`, for example `codez-v0.153.4-r1`. The release workflow builds all four platform archives, publishes one checksum manifest, and caches Cargo dependencies and compiler outputs between runs. Upstream upgrades are reviewed and merged through pull requests before a release tag is created.
