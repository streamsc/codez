...
//! interactive-session layers. Direct `codez login` intentionally does less: it preserves the
...
//! into a one-shot CLI command while still producing a durable `codez-login.log` artifact that
...
/// Installs a small file-backed tracing layer for direct `codez login` flows.
///
...
/// command produce a durable `codez-login.log` artifact without coupling it to the TUI's broader
...
fn init_login_file_logging(config: &Config) -> Option<WorkerGuard> {
    ...
    let log_path = log_dir.join("codez-login.log");
    ...
    // Direct `codez login` otherwise relies on ephemeral stderr and browser output.
...
fn print_login_server_start(actual_port: u16, auth_url: &str) {
    eprintln!(
        "Starting local login server on http://localhost:{actual_port}.\nIf your browser did not open, navigate to this URL to authenticate:\n\n{auth_url}\n\nOn a remote or headless machine? Use `codez login --device-auth` instead."
    );
}
...
pub fn read_api_key_from_stdin() -> String {
    read_stdin_secret(
        "--with-api-key expects the API key on stdin. Try piping it, e.g. `printenv OPENAI_API_KEY | codez login --with-api-key`.",
...
pub fn read_access_token_from_stdin() -> String {
    read_stdin_secret(
        "--with-access-token expects the access token on stdin. Try piping it, e.g. `printenv CODEX_ACCESS_TOKEN | codez login --with-access-token`.",
...
/// `codez login` working in environments where device-code may be disabled/feature-gated.
