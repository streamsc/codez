...
fn write_footer(out: &mut String, options: HumanOutputOptions) {
    if options.show_details {
        ...
        let _ = writeln!(
            ...
            "{}",
            dim(
                "Run codez doctor without --summary for detailed diagnostics.",
                options
            )
        );
        let _ = writeln!(
...
Notes
   ...
   ✗ auth         token expired - Run `codez login`.
─────────────────────────────────────────────────────────────
...
Notes
   [!!] terminal     narrow terminal
   [XX] auth         token expired - Run `codez login`.
-------------------------------------------------------------
