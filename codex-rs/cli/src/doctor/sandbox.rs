...
#[cfg(target_os = "windows")]
const WINDOWS_SETUP_REMEDIATION: &str = "run codez sandbox setup --elevated --user <end-user> --codex-home <authoritative-home> from an elevated shell";
                ...
                if let Some(report) = report {
                    let remediation = match report.code {
                        ...
                        | SetupErrorCode::HelperUnknownError => {
                            "repair or reinstall the Codez CLI from an approved distribution"
                        }
                    };
                    check
                        .details
