    ...
    if !scan.files.is_empty() {
        check = check
            .issue(
                ...
                .remedy("Start Codez with no state DB present so startup backfill can create it from rollout files."),
        )
            .remediation(
                "Start Codez with no state DB present so startup backfill can create it from rollout files.",
            );
    }
    ...
    async fn thread_inventory_check_warns_for_missing_stale_and_mismatched_rows() {
        ...
        assert!(check.issues.iter().all(|issue| {
            !issue
                .remedy
                .as_deref()
                .is_some_and(|remedy| remedy.starts_with("Restart Codez"))
        }));
        assert!(
            check
                .details
