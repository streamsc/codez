...
const INSTALL_URL: &str =
    "https://github.com/streamsc/codez/releases/latest/download/install-codez.sh";
...
pub(crate) fn reexec_managed_updater(managed_codex_bin: &std::path::Path) -> Result<()> {
    ...
    Err(err).with_context(|| {
        format!(
            "failed to replace updater with managed Codez binary {}",
...
async fn install_latest_standalone(http: &RouteAwareClientPool) -> Result<()> {
    ...
    let mut child = Command::new("/bin/sh")
        ...
        .stderr(Stdio::null())
        .spawn()
        .context("failed to invoke standalone Codez updater")?;
    let mut stdin = child
        .stdin
        .take()
        .context("standalone Codez updater stdin was unavailable")?;
    stdin
        .write_all(&script)
        .await
        .context("failed to pass standalone Codez updater to shell")?;
    drop(stdin);
    let status = child
        .wait()
        .await
        .context("failed to wait for standalone Codez updater")?;

    if status.success() {
        Ok(())
    } else {
        anyhow::bail!("standalone Codez updater exited with status {status}")
    }
}
...
async fn fetch_installer_script(http: &impl InstallerHttp) -> Result<Vec<u8>> {
    match http.get(INSTALL_URL).await? {
        ...
        InstallerResponse::Unsuccessful { status } => {
            anyhow::bail!("standalone Codez updater request failed with status {status}")
        }
    }
}
...
impl InstallerHttp for RouteAwareClientPool {
    async fn get(&self, url: &str) -> Result<InstallerResponse> {
        let response = RouteAwareClientPool::get(self, url)
            .send()
            .await
            .context("failed to fetch standalone Codez updater")?;
        ...
        let body = response
            .bytes()
            .await
            .context("failed to read standalone Codez updater")?
            .to_vec();
