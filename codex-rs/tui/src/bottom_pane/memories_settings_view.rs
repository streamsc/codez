...
impl MemoriesSettingsView {
    ...
    fn settings_header(&self) -> ColumnRenderable<'_> {
        ...
        header.push(Line::from(
            "Choose how Codez uses and creates memories. Changes are saved to config.toml".dim(),
        ));
        header
    }
    ...
    fn reset_confirmation_header(&self) -> ColumnRenderable<'_> {
        ...
        header.push(Line::from(
            "This clears local memory files and rollout summaries for the current Codez home."
                .dim(),
        ));
        header
    }
