...
fn startup_delays_composer_for_homes_without_authentication_state() -> std::io::Result<()> {
    ...
    let daemon_directory = codex_home.join("codez-app-server");
    ...
    std::fs::write(daemon_directory.join("control.sock"), "")?;
    assert!(!should_delay_startup_composer_for_first_login(
        &codex_home,
        Ok(system_config_path.clone()),
    ...
    std::fs::remove_file(daemon_directory.join("control.sock"))?;
